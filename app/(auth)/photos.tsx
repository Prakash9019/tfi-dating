...
const PhotoCard = ({ uri, status, index, translateX }: any) => {
  const { width } = useWindowDimensions();
  const screenCenter = width / 2;

  const animatedStyle = useAnimatedStyle(() => {
    const cardCenterPosition =
      index * ITEM_WIDTH + translateX.value;

    const distanceFromCenter = Math.abs(
      screenCenter - cardCenterPosition - CARD_WIDTH / 2
    );
...
export default function PhotosScreen() {
  ...
  useEffect(() => {
    ...
    translateX.value = withRepeat(
      withTiming(-SINGLE_SET_WIDTH, {
        duration: 3500, // Speed of the slide (lower = faster)
  ...
  const handleUpload = async () => {
    Alert.alert(
      "Add a pic",
      "It can be anything, but pics of you get a verification boost on TfiFan!",
      [
      ...
      <View style={styles.footer}>
        ...
        <View style={styles.warning}>
          ...
          <Text style={styles.warningText}>
            Avoid NSFW or inappropriate content
          </Text>
        </View>
      </View>
    </SafeAreaView>
